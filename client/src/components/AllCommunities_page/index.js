...
class AllCommunities extends Component {
  constructor(props) {
    super(props);
    this.handleChangeCommunityNameText = this.handleChangeCommunityNameText.bind(
      this
    );
  }
  state = {
  ...
  render(newItem) {
    ...
    return (
      <div className="container">
        <h3 className="updateTitle">Add/Update Communities</h3>

        <table className="table table-striped table-dark tableUpdate">
          <thead>
            <tr>
              <th scope="col">#</th>
          ...
          <tbody>
            {this.state.results.map((community, index) => {
              return (
                <tr key={index}>
                  ...
                  <td>
                    <input
                      type="text"
                      onChange={(event) =>
                        this.handleChangeCommunityNameText(event, index)
                      }
                      value={this.state.results[index].name}
                    ></input>
                  </td>
                  <td>
                    <button
                      onClick={() =>
                        this.updateCommunityName(
                          community._id,
            ...
            <tr key={this.state.results.length + 1}>
              ...
              <td>
                <button
                  onClick={() => this.addNewCommunity(this.state.newComm)}
                >
                  Add
                </button>
              </td>
